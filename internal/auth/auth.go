...
func (a *Authenticator) Login(ctx context.Context, w http.ResponseWriter, code, state string) (*store.Streamer, error) {
	...
	sid, err := a.store.CreateAuthSession(streamer.ID, sessionTTL)
	if err != nil {
		return nil, err
	}
	a.setCookie(w, sid)
...
// Authenticate resolves the streamer for a request, or ErrUnauthenticated.
func (a *Authenticator) Authenticate(r *http.Request) (*store.Streamer, error) {
	c, err := r.Cookie(cookieName)
	if err != nil || c.Value == "" {
		return nil, ErrUnauthenticated
	}
	st, err := a.store.GetValidAuthSession(c.Value)
...
// WithStreamer returns a copy of ctx carrying the streamer.
func WithStreamer(ctx context.Context, s *store.Streamer) context.Context {
	return context.WithValue(ctx, ctxKey{}, s)
}

// StreamerFrom extracts the streamer placed by RequireStreamer.
...
// and otherwise injects the streamer into the request context. It also applies a
// lightweight same-origin (CSRF) guard to unsafe methods.
func (a *Authenticator) RequireStreamer(next http.HandlerFunc) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		st, err := a.Authenticate(r)
		if err != nil {
			http.Error(w, `{"error":"unauthorized"}`, http.StatusUnauthorized)
			return
		}
		next(w, r.WithContext(WithStreamer(r.Context(), st)))
	}
}
